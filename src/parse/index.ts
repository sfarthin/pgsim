...
const findNextToken = (str: string, pos: number) => {
  ...
  return { start: pos + whitespace.length, end: pos + token.length + 1 };
};
...
const getFriendlyErrorMessage = (
  ...
  const expected = result.expected.filter((v) => !['"/*"', '"--"'].includes(v));
  const message = `Expected ${expected.join(" or ")}`;

  const lines = str.split("\n");
  const nextToken = findNextToken(str, result.pos + 1);
