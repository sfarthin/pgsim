...
import { sequence, __, combineComments, constant, or } from "./util";
import { RawValue, AExprKind } from "../types";
import { rowExpr } from "./rowExpr";

const aExpr_equal = connectRawCondition(
...
const aExpr_in = connectRawCondition(
  sequence([__, constant("in"), __, rowExpr]),
  (c1, v) => {
    return {
      codeComment: combineComments(
        c1.codeComment,
